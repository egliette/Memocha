...
def test_create_message_via_crud(db_session):
...
def test_get_message_history_by_session(db_session):
    session = session_crud.create_session(db_session)
...
def test_message_history_pagination(db_session):
    session = session_crud.create_session(db_session)
    ...
    messages_page1 = message_crud.get_messages_by_session(
        db_session, session.id, skip=0, limit=2
    )
    assert len(messages_page1) == 2

    messages_page2 = message_crud.get_messages_by_session(
...
def test_message_history_empty_session(db_session):
    session = session_crud.create_session(db_session)
    messages = message_crud.get_messages_by_session(db_session, session.id)
    assert len(messages) == 0


def test_message_history_isolation(db_session):
    session1 = session_crud.create_session(db_session)
    session2 = session_crud.create_session(db_session)
    ...
    messages1 = message_crud.get_messages_by_session(db_session, session1.id)
    messages2 = message_crud.get_messages_by_session(db_session, session2.id)
